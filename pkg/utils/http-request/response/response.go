...
import (
	...
	enumErrors "github.com/ZupIT/horusec-devkit/pkg/utils/http-request/response/enums"
...
func (h *HTTPResponse) ErrorByStatusCode() error {
	body, _ := h.GetBody()
	switch {
	case h.response.StatusCode >= http.StatusInternalServerError:
	...
	case h.response.StatusCode >= http.StatusBadRequest && h.response.StatusCode < http.StatusInternalServerError:
		logger.LogError(enumErrors.ErrHTTPResponse.Error(), errors.New(string(body)), h.mapResponseStatus())
